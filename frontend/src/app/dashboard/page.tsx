...
import { HardHat, Briefcase, Search, ShieldCheck, LogOut, User as UserIcon, Building, CheckCircle, XCircle, Clock, MapPin, Sparkles, AlertCircle, Bot } from 'lucide-react';
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <section className="md:col-span-2 bg-surface-cardLight dark:bg-surface-cardDark border-2 border-black rounded-brutal p-6 shadow-brutal-light dark:shadow-brutal-dark flex flex-col md:flex-row gap-6 items-center">
            ...
            <div className="flex-1 w-full text-center md:text-left">
              ...
              <p className="font-bold opacity-80 mb-4">{profile.specialization || 'Профиль в стадии заполнения...'}</p>
              
              {/* Шкала доверия (Gamification) */}
          ...
          {/* НОВЫЙ БЛОК: ИИ-Диспетчер (CTA) */}
          <section onClick={() => router.push('/onboarding')} className="md:col-span-1 bg-gradient-to-br from-brand to-brand-dark border-2 border-black rounded-brutal p-6 shadow-brutal-light cursor-pointer hover:translate-x-1 hover:-translate-y-1 hover:shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] transition-all group relative overflow-hidden flex flex-col justify-center items-center text-center">
            ...
            <p className="text-sm font-bold text-black/80">
              {profile.verification_level < 1 ? 'Пройти верификацию' : (isWorker ? 'Обновить навыки и стаж' : 'Опубликовать новый заказ')}
            </p>
          </section>
        </div>
                ...
                <div className="grid gap-4">
                  {feedProjects.length === 0 ? (
                    <div className="p-10 border-2 border-dashed border-gray-400 rounded-brutal flex flex-col items-center text-center">
                      ...
                      <p className="text-sm opacity-70">Заказчики еще формируют ТЗ.</p>
                    </div>
                ...
                <div className="grid gap-6">
                  {dashboardData.projects.length === 0 ? (
                    <div className="p-10 border-2 border-dashed border-gray-400 rounded-brutal text-center">
                      ...
                      <Button onClick={() => router.push('/onboarding')} size="sm">Создать первый заказ через ИИ</Button>
                    </div>
                    ...
                    <div key={proj.id} className="border-2 border-black rounded-brutal overflow-hidden shadow-sm">
                      ...
                      <div className="p-5 bg-surface-light dark:bg-surface-dark">
                        <h4 className="text-sm font-black uppercase text-gray-500 mb-3 border-b-2 border-gray-200 pb-2">Отклики специалистов ({proj.bids.length})</h4>
          ...
          <div className="md:col-span-4 space-y-6">
            ...
            {isWorker && dashboardData?.bids && (
              <div className="bg-white dark:bg-surface-cardDark border-2 border-black rounded-brutal p-6 shadow-skeuo-inner-light">
                <h3 className="font-black uppercase text-sm mb-4 border-b-2 border-black pb-2">Активные отклики</h3>
                <div className="space-y-4">
