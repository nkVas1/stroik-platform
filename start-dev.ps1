# =============================================================================
#  СТРОИК — единый скрипт запуска dev-окружения для Windows / PowerShell
...
Write-Host "  ╚══════╝   ╚═╝   ╚═╝  ╚═╝ ╚═════╝ ╚═╝╚═╝  ╚═╝" -ForegroundColor White
Write-Host "    Интеллектуальная платформа стройки  •  dev-launcher" -ForegroundColor Gray
...
if (-not $SkipDeps) {
    ...
    Push-Location $backendDir
    # Editable-install подтянет [project.dependencies] (включая google-genai, pydantic-settings)
...
if ($ollamaExists -and -not $SkipOllama) {
    ...
    try {
        ...
        if ($modelList -match "llama3") {
            ...
            Write-Step "Скачиваю модель llama3 (~4.7 ГБ, один раз)…"
            & ollama pull llama3
    ...
    Write-Warn "Ollama недоступен — fallback на Llama3 работать не будет (Gemini продолжит работу)"
}
...
Write-Host '╔══════════════════════════════════════════╗' -ForegroundColor DarkCyan;
Write-Host '║  СТРОИК · Backend (FastAPI)              ║' -ForegroundColor Cyan;
...
Write-Host '╔══════════════════════════════════════════╗' -ForegroundColor DarkCyan;
Write-Host '║  СТРОИК · Frontend (Next.js)             ║' -ForegroundColor Cyan;
