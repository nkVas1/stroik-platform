...
// Типизация сообщения строго по контракту с нашим FastAPI (Pydantic модель Message)
type Message = {
...
export default function ChatWindow() {
  ...
  // Инициализируем чат приветственным сообщением от ассистента
  ...
  const sendMessage = async (e?: React.FormEvent) => {
    ...
    try {
      // Отправляем запрос на наш локальный бэкенд
      ...
      // Если онбординг завершен, перенаправляем через 2 секунды
      if (data.is_complete) {
        setIsFinished(true);
        setTimeout(() => {
            ...
            <div className={cn(
              ...
              msg.role === 'user' 
                ...
                : msg.role === 'assistant'
                  ...
                  : "bg-red-100 text-red-900 rounded-tl-none" // Системная ошибка
            )}>
              {msg.content}
            </div>
          </div>
